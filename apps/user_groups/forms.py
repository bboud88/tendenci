...
class MailchimpExportForm(forms.Form):
    """Handles Message Form"""
    to_list = forms.ChoiceField(label=_(u'Choose MailChimp List to Export To: (emails will be appended to the existing list)'))

    def __init__(self, *args, **kwargs):
        kwargs.pop('request')
        list_results = kwargs.pop('list_results')
        super(MailchimpExportForm, self).__init__(*args, **kwargs)

        self.fields['to_list'] = forms.ChoiceField(choices=list_results,label=_(u'Choose MailChimp List to Export To (emails will be appended to the existing list)'))
