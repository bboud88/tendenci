...
from tendenci.apps.user_groups.forms import MailchimpExportForm
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if form.cleaned_data['is_test']:
            ...
            msg = EmailMessage('Request Callback',
                               'Here is the message.', to=['blake.boudreau@gmail.com'])
            msg.send()
...
@superuser_required
def mailchimp_export(request, group_slug, template_name='user_groups/mailchimp-export.html'):
    """
    Send a message to the group
    """
    from tendenci.apps.emails.models import Email

    api_url = settings.MAILCHIMP_API_URL+'/lists/'
    api_key = settings.MAILCHIMP_API_KEY
    headers = {'Content-Type': 'application/json', 'Authorization' : api_key}
    response = requests.get(api_url, headers=headers);
    jsonInput = json.loads(response.text);
    ...
    form = MailchimpExportForm(request.POST or None,
        request=request,
        list_results=list_results)

    if request.method == 'POST' and form.is_valid():
        to_list = form.cleaned_data['to_list']
        email_dictionary = {"members": [], "update_existing": True}
        email_count = 0
        for member in members:
            email_dictionary["members"].append({"email_address": member.member.email, "status": "subscribed"})
            email_count += 1

        if email_count > 0:
            json_data = json.dumps(email_dictionary)
            api_url = settings.MAILCHIMP_API_URL+'/lists/'+to_list
            api_key = settings.MAILCHIMP_API_KEY
            headers = {'Content-Type': 'application/json', 'Authorization': api_key}
...
def group_members_export(request, group_slug, export_target='all'):
    ...
    users = group.members.filter(
                ...
                ).order_by('group_member__member_id')
    email_dictionary = {"members":[], "update_existing": True}
    email_count = 0
    for user in users:
        if hasattr(user, 'email'):
            email = getattr(user, 'email')
            email_dictionary["members"].append({"email_address": email, "status": "subscribed"})
            email_count += 1

    if email_count > 0:
        json_data = json.dumps(email_dictionary)
        api_url = settings.MAILCHIMP_API_URL+'/lists/74a3ebf7b0'
        api_key = settings.MAILCHIMP_API_KEY
        headers = {'Content-Type': 'application/json', 'Authorization': api_key}
        response = requests.post(api_url, data=json_data, headers=headers)
        print(response.content)
