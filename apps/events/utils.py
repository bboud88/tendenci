...
def create_member_registration(user, event, form):
    ...
    registrants = form.cleaned_data['members']
    old_registrants = event.registrants()

    for old_r in old_registrants:
        still_registered = registrants.filter(pk=old_r.user.pk)
        if not still_registered:
          event.registrants().filter(user=old_r.user).delete()
          #test what happens when there were no registrants to begin with - do we get an error because calling filter?

    for r in registrants:
        reguser = User.objects.filter(username=r)
        if reguser:
            [member] = Profile.objects.filter(user=reguser,
